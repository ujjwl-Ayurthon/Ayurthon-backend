...
router.get('/check/:test_token/:telegram_username', async (req, res) => {
  try {
    ...
    const student = await Student.findOne({ telegram_username: telegram_username.toLowerCase() });
...
router.post('/submit', async (req, res) => {
  try {
    ...
    const higherRank = await Result.countDocuments({
      ...
      $or: [
        { score: { $gt: score } },
        { score: score, time_taken_seconds: { $lt: time_taken_seconds || 0 } }
      ]
    });
    ...
    // Fetch wrong + skipped question details
    ...
    const skippedQuestionsDetail = await Question.find({ _id: { $in: skippedQuestions } })
      .select('text options correct_answer explanation reference type');
    ...
    res.json({
      ...
      result: {
        ...
        skipped_questions: skippedQuestionsDetail,
        answers:           processedAnswers
      }
    });
...
router.get('/sheet/:test_id', adminAuth, async (req, res) => {
  try {
    const test = await Test.findById(req.params.test_id).populate('questions', 'text correct_answer');
    const results = await Result.find({ test_id: req.params.test_id })
      .sort({ score: -1, time_taken_seconds: 1 })
      .populate('student_id', 'name telegram_username');
    ...
    const sheet = results.map((r, i) => ({
      ...
      responses:         r.answers.map(a => ({
        ...
        selected:        a.selected_option || 'Skipped',
